...
def test_export_monthly_xlsx_creates_date_blocks(tmp_path) -> None:
    ...
    assert sheet["E2"].value == "19.05.2026"
    assert sheet["E3"].value == "Цена без скидки (регулярная)"
    assert sheet["F3"].value == "Цена со скидкой"
    assert sheet["A4"].value == "Вайлдберриз"
    assert sheet["E4"].value == 159.0
    assert sheet["F4"].value == 101.0
...
def test_build_sheet_values_includes_date_row(tmp_path) -> None:
    ...
    assert values[0][4] == "19.05.2026"
    assert values[0][5] is None
    assert values[1][4] == "Цена без скидки (регулярная)"
